...
{
    public class ManagePPModel : PageModel
    { 
            ...
            public ManagePPModel(UserManager<ApplicationUser> userManager,
            ...
            public IActionResult OnPostAddParticipant(Guid id, Guid policyTypeid, Guid policyid)
            {
                try
                {
                    ...
                    policyBeneficiary.AddedOn = DateTime.Now;
                    policyBeneficiary.AddedBy = AddedBy;
                    _policyBeneficiaryRepository.ProposeAdditionalLifeAssured(policyBeneficiary,RequestID);
                    BeneficiariesDT = _policyBeneficiaryRepository.GetBeneficiaryList(policyid);
                    AdditionalLifeDT = _policyBeneficiaryRepository.GetAdditionalLifeAssured(policyid,RequestID);
                    Insured = false;
                    IsBeneficiary = false;
        ...
        public IActionResult OnPostNext(Guid id, Guid policyTypeid, Guid policyid)
        {
            try
            {
                using (TransactionScope TS = new TransactionScope())
                {
                   ...
                   PolicyDates.AgentSignedDate > DateTime.Today || PolicyDates.DeductionStartDate < DateTime.Today)
                    ...
                    if (!string.IsNullOrEmpty(AgentCodes))
                    {
					...
					_policyRepository.InsertPolicyServicingRequests(PolicyID, RequestID, 6, 5, "", DateTime.Now, 0);
            ...
            return Redirect("PPDetails?id=" + id + "&policytypeid=" + policyTypeid + "&policyid=" + policyid + "&reqid=" + RequestID);
        }
        ...
        public IActionResult OnPostArchiveBeneficiary(int id, Guid policyTypeid, Guid policyid, Guid memberID)
        {
            ...
            _policyBeneficiaryRepository.ProposeBeneficiaryArchive(id, RequestID, archivedBy);
           return Redirect("ManagePP?id=" + memberID + "&policytypeid=" + policyTypeid + "&policyid=" + policyid + "&reqid=" + RequestID);
        }
        ...
        public IActionResult OnPostArchiveBeneficiaryLine(int lineid)
        {
            string addedBy = _userManager.GetUserId(User).ToString();
            _policyBeneficiaryLineRepository.ProposeBeneficiaryLineArchive(lineid, RequestID, addedBy);
            return Redirect("ManagePP?id=" + MemberID + "&policytypeid=" + PolicyTypeID + "&policyid=" + PolicyID + "&reqid=" + RequestID);
        }
    }

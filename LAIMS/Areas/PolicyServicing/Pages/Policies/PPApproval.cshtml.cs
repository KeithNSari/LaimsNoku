...
{
    public class PPApprovalModel : PageModel
    {
        ...
        public DataTable ExistingIntermediariesDT { get; set; }
        ...
        public PPApprovalModel(UserManager<ApplicationUser> userManager,
        ...
        public IActionResult OnGet(Guid id, Guid policyTypeid, Guid policyid, Guid reqid)
        {
            ...
            AdditionalLifeDT = _policyBeneficiaryRepository.GetAdditionalLifeAssured(policyid, RequestID);
            MainLifeDT = _policyBeneficiaryRepository.GetMainLifeAssured(policyid);
            PolicyBeneficiaryLineDetails = _policyBeneficiaryLineRepository.GetPolicyBeneficiaryLineDetailsByRequestID(policyid, RequestID);
            //payments
            ...
            AddRequestPremiumHeader(AddedBy);
            if (PremiumPayerID > 0)
            {
                PremiumPayer = _memberRepository.GetMemberById(PremiumPayerID);
                PolicyPremium = _policyPremiumRepository.GetPolicyPremiumByRequest(RequestID);
                PolicyPremiumID = PolicyPremium.ID;
                PremiumDetails = _policyPremiumRepository.GetPremiumDetails(policyid, PolicyPremiumID);
            }
            PolicyDates = _policyPremiumRepository.GetPolicyPremiumDates(PolicyPremiumID);
            LoadStopOrderProvidersSelectList();
            LoadCategoriesSelectList();
            LoadDebitOrderProvidersSelectList();
            IntermediariesDT = _policyPremiumRepository.GetAgents(PolicyPremiumID);
            ExistingIntermediariesDT = _policyPremiumRepository.GetInitialPremiumAgents(PolicyID);
            return Page();
        }
        ...
        private void AddRequestPremiumHeader(string AddedBy)
        {
            PolicyPremium policyPremium = new PolicyPremium();
            policyPremium.HeaderID = PolicyID;
            ...
            policyPremium.AddedBy = AddedBy;
            policyPremium.AddedOn = DateTime.Now;
            PolicyPremiumID = _policyPremiumRepository.AddPolicyPremium(policyPremium, RequestID);
        }
        public IActionResult OnPostAddProductPremium(Guid id, Guid policyTypeid, Guid policyid)
        {
            ...
            PolicyBeneficiaryLine.RequestID = RequestID;
            PolicyBeneficiaryLine.Approved = 0;
        ...
        public IActionResult OnPostNext(Guid id, Guid policyTypeid, Guid policyid)
        {
            ...
            using (TransactionScope TS = new TransactionScope())
            {
               ...
               PolicyDates.AgentSignedDate > DateTime.Today || PolicyDates.DeductionStartDate < DateTime.Today)
                ...
                _policyPremiumLineRepository.UpdatePolicyPremiumDates(PolicyPremiumID, PolicyDates);

                _policyPremiumLineRepository.UpdatePolicyPremiumLines(PolicyPremiumID);

                _policyPremiumRepository.UpdatePolicyPremiumIntermediaries(PolicyPremiumID, AgentCodes);
            ...
            //    return RedirectToPage("/Error", new { errorMessage = "An error occurred during processing. " + ex.Message, returnUrl = ReturnUrl });
            //} 
            return Redirect("PPApproval?id=" + id + "&policytypeid=" + policyTypeid + "&policyid=" + policyid + "&reqid=" + RequestID);
        }
        ...
        public IActionResult OnPost()
        {
            ...
            if (StatusID == 10)
            {
                using (TransactionScope TS = new TransactionScope())
                {
                    _policyRepository.PolicyServicingMessagesAdd(PolicyID, MemberID, 5, "REF: Policy premiums updated.", addedBy);
                    _policyBeneficiaryLineRepository.PPDetailsApproveAdditionalComponents(RequestID, addedBy);
                    _policyPremiumLineRepository.UpdateMainPremiumLines(PolicyID, RequestID);
                    _policyPremiumLineRepository.UpdatePolicyPremiumLines(PolicyPremiumID);
                    _policyPremiumRepository.UpdatePremiumAmount(PolicyID, PolicyPremiumID);
                    _policyPremiumRepository.ApprovePremium(PolicyPremiumID);
            ...
            else if (StatusID == 9)
            {
				...
				_policyRepository.PolicyServicingMessagesAdd(PolicyID, MemberID, 5, "REF:  Policy premiums on policy rejected.", addedBy);
            }
            else if (StatusID == 8)
            {
				_policyRepository.UpdatePolicyServicingRequests(RequestID, 8);
				_policyRepository.PolicyServicingMessagesAdd(PolicyID, MemberID, 5, "REF: More information requested for  policy premiums update.", addedBy);
            }
            return Redirect("PolicyDetails?id=" + MemberID + "&policytypeid=" + PolicyTypeID + "&policyid=" + PolicyID); 
        }
    }
}
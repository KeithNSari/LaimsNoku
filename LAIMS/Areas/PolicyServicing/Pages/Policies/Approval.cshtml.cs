...
namespace LAIMS.Areas.PolicyServicing.Pages.Policies
{
    public class ApprovalModel : PageModel
    {
        ...
        public ApprovalModel(UserManager<ApplicationUser> userManager,
        ...
        public IActionResult OnPost()
        {
            ...
            if (StatusID == 10)
            {
                _policyRepository.PolicyServicingMessagesAdd(PolicyID, MemberID, 4, "REF: Beneficiary update approved on policy.", addedBy);
                _policyBeneficiaryRepository.UpdateBeneficiaryListFromCopy(PolicyID, RequestID);
                _policyBeneficiaryRepository.ArchivePBLSplit(PolicyID, addedBy);
                _policyBeneficiaryRepository.UpdatePBLSplitFromStaging(RequestID);
            }
            else if (StatusID == 9)
            {
                _policyRepository.PolicyServicingMessagesAdd(PolicyID, MemberID, 4, "REF: Beneficiary update on policy rejected.", addedBy);
            }
            else if (StatusID == 8)
            {
                _policyRepository.PolicyServicingMessagesAdd(PolicyID, MemberID, 4, "REF: More information requested on beneficiary update.", addedBy);
            }

...
{
    public class PRateFilesModel : PageModel
    {
        private readonly IPaymentRepository _paymentRepository;
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly IWebHostEnvironment _environment;
        private readonly IUploadData _dataUpload;
        public DataTable PremiumRateBatchesDT { get; set; }

        [BindProperty]
        public IFormFile Upload { get; set; }
        [BindProperty ]
        public DateTime EffectiveDate { get; set; }
        public PRateFilesModel(UserManager<ApplicationUser> userManager, IWebHostEnvironment environment,
        ...
        public void OnGet()
        {
            try
            {
                ...
                PremiumRateBatchesDT = _paymentRepository.GetLatestPremiumRatesHeaders();
            }
            catch (Exception ex)
            {
            ...
            //{
                if (Upload != null)
                {
                    ...
                    using (TransactionScope TS = new TransactionScope())
                    {
                        ...
                        _paymentRepository.UploadPremiumRates(fileContents.DataDT, mediaUploadID,EffectiveDate, AddedBy); 
                        TS.Complete();
                    }
                }
                Response.Redirect("PRateFiles");
            //}
            //catch (Exception ex)
            //{
            //    ViewData["ErrorMessage"] = "An error occurred: " + ex.Message;

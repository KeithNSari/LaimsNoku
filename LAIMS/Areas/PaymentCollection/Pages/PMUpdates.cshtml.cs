...
namespace LAIMS.Areas.PaymentCollection.Pages
{
    ...
    public class PMUpdatesModel : PageModel
    {
		...
		public DataTable PolicyDT { get; set; }
        public PMUpdatesModel(UserManager<ApplicationUser> userManager,
        ...
        {
            SearchTerm = search;
            SearchPageUrl = "pmupdates";
            if (!string.IsNullOrEmpty(SearchTerm))
            {
				PolicyDT = _policyRepository.GetBy(SearchTerm);
				ResultsCount = PolicyDT.Rows.Count;
                if (ResultsCount > 0)
				{
					DataRow DR = PolicyDT.Rows[0];
					Guid memberId = Guid.Parse(DR["MemberUID"].ToString()); 
				    Guid policyTypeid = Guid.Parse(DR["PolicyType"].ToString());
				    Guid policyid = Guid.Parse(DR["ID"].ToString());
					LoadPolicy(memberId, policyTypeid, policyid);
					BillingDay= _policyPremiumRepository.GetCurrentBillingDay(policyid);
                }
            }
        }
        private void LoadPolicy(Guid id, Guid policyTypeid, Guid policyid)
		{ 
			PolicyID = policyid;
		...
		public IActionResult OnPostAddPremiumPayer(Guid id, Guid policyTypeid, Guid policyid)
		{
			using (TransactionScope TS = new TransactionScope())
			{
				try
				{
                    ...
                    _policyRepository.PolicyServicingMessagesAdd(PolicyID, ProposerID, 3, "Please note: the premium payer for this policy has been edited. If this change is unexpected, please contact us!", AddedBy);
                    TS.Complete();
					TempData["PostbackSuccess"] = true;
					return Redirect("PMUpdates?handler=search&search=" + SearchTerm);
				}
				catch (Exception ex)
				{
        ...
        public IActionResult OnPostAddStopOrder(Guid id, Guid policyTypeid, Guid policyid)
        {
            using (TransactionScope TS = new TransactionScope())
            {
                try
                {
                    ...
                    _policyRepository.PolicyServicingMessagesAdd(PolicyID, ProposerID, 3, "Please note: a new stop order has been added for this policy. If this change is unexpected, please contact us!", AddedBy);
                    TS.Complete();
                    TempData["PostbackSuccess"] = true;
                    return Redirect("PMUpdates?handler=search&search=" + SearchTerm);
                }
        ...
        public IActionResult OnPostAddDebitOrder(Guid id, Guid policyTypeid, Guid policyid)
		{
			using (TransactionScope TS = new TransactionScope())
			{
				try
				{
					...
					int billingDay = _policyPremiumRepository.GetCurrentBillingDay(policyid);
					int currentDay = DateTime.Today.Day;
					//if (billingDay <= (currentDay + 2)) throw new Exception("A lead time of more than 2 days is required for this change!");
					_policyPremiumRepository.UpdateBillingDay(policyid,BillingDay);
					...
					CheckPaymentMethodRules(PolicyTypeID, PolicyID, id, (int)PolicyPremium.PaymentMethodID);
					_policyPremiumRepository.UpdatePaymentMethod(PolicyPremium, policyid);
                    _policyRepository.PolicyServicingMessagesAdd(PolicyID, ProposerID, 3, "Please note: a new debit order has been added for this policy. If this change is unexpected, please contact us!", AddedBy);
		...
		public IActionResult OnPostAddDirectPayment(Guid id, Guid policyTypeid, Guid policyid)
		{
			try
			{
                ...
                _policyPremiumRepository.UpdateBillingDay(policyid, BillingDay);
                TempData["PostbackSuccess"] = true;
                _policyRepository.PolicyServicingMessagesAdd(PolicyID, ProposerID, 3, "Please note: method of payment has been changed to direct payment. If this change is unexpected, please contact us!", AddedBy);
